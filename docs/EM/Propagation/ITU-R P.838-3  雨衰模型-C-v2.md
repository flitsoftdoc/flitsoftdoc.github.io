...
a）由于降雨强度的关系，有必要计算由于下雨带来的衰减，
建议

## 1 使用下列程序。

具体的衰减 $\gamma_{R}(\mathrm{~dB} / \mathrm{km})$ 可以从降雨强度 $R(\mathrm{~mm} / \mathrm{h})$ 的幂次律关系中算出：

$$
\begin{equation*}
\gamma_{R}=k R^{\alpha} \tag{1}
\end{equation*}
$$

作为频率范围为 1 至 1000 GHz 的频率 $f(\mathrm{GHz})$ 的函数，系数 $k$ 和 $\alpha$ 的值由下列等式确定，这些等式是通过从离散计算中获得的从曲线拟合到幂次律系数来推出的：

$$
\begin{align*}
\log _{10} k & =\sum_{j=1}^{4}\left(a_{j} \exp \left[-\left(\frac{\log _{10} f-b_{j}}{c_{j}}\right)^{2}\right]\right)+m_{k} \log _{10} f+c_{k}  \tag{2}\\
\alpha & =\sum_{j=1}^{5}\left(a_{j} \exp \left[-\left(\frac{\log _{10} f-b_{j}}{c_{j}}\right)^{2}\right]\right)+m_{\alpha} \log _{10} f+c_{\alpha} \tag{3}
\end{align*}
$$
...
f: & \text { 频率 }(\mathrm{GHz}) \\
k: & k_{H} \text { 或 } k_{V} \\
\alpha: & \alpha_{H} \text { 或 } \alpha_{V^{\circ}}
\end{array}
$$

水平极化的系数 $k_{H}$ 的常数值在表1中给出，而垂直极化的系数 $k_{V}$ 的常数值在表2中给出。表3给出了水平极化的系数 $\alpha_{H}$ 的常数值，而表4给出了垂直极化的系数 $\alpha_{V}$ 的常数值。

表 1 $\boldsymbol{k}_{\boldsymbol{H}}$ 系数

| $\boldsymbol{j}$ | $\boldsymbol{a}_{\boldsymbol{j}}$ | $\boldsymbol{b}_{\boldsymbol{j}}$ | $\boldsymbol{c}_{\boldsymbol{j}}$ | $\boldsymbol{m}_{\boldsymbol{k}}$ | $\boldsymbol{c}_{\boldsymbol{k}}$ |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
表 2 $\boldsymbol{k}_{\boldsymbol{V}}$ 系数

| $\boldsymbol{j}$ | $a_{j}$ | $\boldsymbol{b}_{\boldsymbol{j}}$ | $\boldsymbol{c}_{\boldsymbol{j}}$ | $\boldsymbol{m}_{\boldsymbol{k}}$ | $\boldsymbol{c}_{\boldsymbol{k}}$ |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
表 3 $\boldsymbol{\alpha}_{\boldsymbol{H}}$ 系数

| $j$ | $\boldsymbol{a}_{\boldsymbol{j}}$ | $\boldsymbol{b}_{\boldsymbol{j}}$ | $\boldsymbol{c}_{\boldsymbol{j}}$ | $\boldsymbol{m}_{\boldsymbol{\alpha}}$ | $\boldsymbol{c}_{\boldsymbol{\alpha}}$ |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
表 4 $\boldsymbol{\alpha} \boldsymbol{V}$ 系数

| $\boldsymbol{j}$ | $\boldsymbol{a}_{\boldsymbol{j}}$ | $\boldsymbol{b}_{\boldsymbol{j}}$ | $\boldsymbol{c}_{\boldsymbol{j}}$ | $\boldsymbol{m}_{\boldsymbol{\alpha}}$ | $\boldsymbol{c}_{\boldsymbol{\alpha}}$ |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | －0．07771 | 2.33840 | －0．76284 | －0．053739 | 0.83433 |
...
对于线极化和圆极化中所有的路径几何，可以通过等式（2）和（3）中给定的值由下列等式计算出等式（1）中的系数：

$$
\begin{gather*}
k=\left[k_{H}+k_{V}+\left(k_{H}-k_{V}\right) \cos ^{2} \theta \cos 2 \tau\right] / 2  \tag{4}\\
a=\left[k_{H} a_{H}+k_{V} a_{V}+\left(k_{H} a_{H}-k_{V} a_{V}\right) \cos ^{2} \theta \cos 2 \tau\right] / 2 k \tag{5}
\end{gather*}
$$

此处 $\theta$ 是路径斜角，$\tau$ 是相对水平位置的极化斜角（对于圆极化，$\tau=45^{\circ}$ ）。
可以在图1至4中快速查阅系数 $k$ 和 $\alpha$ 的图形显示，表5列出了在给定频率上的系数的数值。

图 1 水平极化的系数

![](https://cdn.mathpix.com/cropped/b551e870-eaf3-480f-b5da-ac6cfa34f411-3.jpg?height=1099&width=1435&top_left_y=977&top_left_x=285)

图 2 水平极化的系数 $\boldsymbol{\alpha}$

![](https://cdn.mathpix.com/cropped/b551e870-eaf3-480f-b5da-ac6cfa34f411-4.jpg?height=955&width=1449&top_left_y=301&top_left_x=317)

图3 垂直极化的系数 $\boldsymbol{k}$

![](https://cdn.mathpix.com/cropped/b551e870-eaf3-480f-b5da-ac6cfa34f411-4.jpg?height=1031&width=1442&top_left_y=1374&top_left_x=317)

图 4 垂直极化的系数 $\boldsymbol{\alpha}$

![](https://cdn.mathpix.com/cropped/b551e870-eaf3-480f-b5da-ac6cfa34f411-5.jpg?height=999&width=1326&top_left_y=367&top_left_x=367)

表5 使用等式（4）、（5）和（1）以及与频率相关的系数估算具体的雨天衰减

| 频率（GHz） | $\boldsymbol{k}_{\boldsymbol{H}}$ | $\boldsymbol{\alpha}_{\boldsymbol{H}}$ | $\boldsymbol{k}_{\boldsymbol{V}}$ | $\alpha_{V}$ |
| :--- | :--- | :--- | :--- | :--- |
...
| 频率（GHz） | $\boldsymbol{k}_{\boldsymbol{H}}$ | $\alpha_{H}$ | $\boldsymbol{k}_{\boldsymbol{V}}$ | $\alpha_{V}$ |
| :--- | :--- | :--- | :--- | :--- |
