...
$h(\mathrm{~km})$ ：圆形轨道上卫星的高度，单位为千米。

$a(\mathrm{~km})$ ：任意卫星轨道的半长轴，单位为千米。

$rng(\mathrm{~km})$ ：从卫星地面投影点（SVP, Subsatellite Vertical Projection）到地平线的空间视距（range），适用于圆形轨道，单位为千米。

$dist(\mathrm{~km})$ ：从SVP到地平线的地球表面距离，适用于圆形轨道，单位为千米。
