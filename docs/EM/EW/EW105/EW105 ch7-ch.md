...
但角 \( E \) 实际大于 \(90^{\circ}\)，因此角 \( E \)（即从地心看卫星的方向与截获站方向之间的夹角）\[= 180^{\circ} - 45.5^{\circ} = 134.5^{\circ}\]，而从截获站观测，卫星相对于本地地平线的仰角（elevation above the horizon）即为 \(45.5^{\circ}\)。
...
因此，角 \(F\)（即从地心看截获站的仰角）\[= \arcsin(0.681) \approx 42.9^{\circ}\]
...
由于这是一个平面直角三角形，满足勾股定理：\[j^2 = k^2 + m^2\]。
...
因此，
\[
\text{角 } K = \arccos(0.955) \approx 17.2^{\circ}
\]
...
下行链路天线主瓣（boresight）与截获站之间的方位角差为：
\[
\Delta AZ = 24.5^{\circ} - 36.0^{\circ} = -11.5^{\circ} \quad (\text{绝对值为 } 11.5^{\circ})
\]

仰角差为：
\[
\Delta EL = 62.5^{\circ} - 58.3^{\circ} = 4.2^{\circ}
\]

截获站方向相对于下行链路天线主瓣的偏角（offset angle）可通过图7.12所示的球面直角三角形确定。
...
因此：
\[
\cos cc = (0.980)(0.997) = 0.977 \quad \Rightarrow \quad cc = \arccos(0.977) \approx 12.3^{\circ}
\]

该偏角（12.3°）远大于卫星下行天线的3 dB波束宽度（3.7°），因此可采用典型的平均旁瓣隔离度（sidelobe isolation）20 dB，来估算下行链路天线在截获站方向上的增益。

于是，下行链路天线指向截获接收机方向的增益为：
\[
33.3\,\mathrm{dBi} - 20\,\mathrm{dB} = 13.3\,\mathrm{dBi}
\]
...
飞机位于天线旁瓣区域，该旁瓣比主瓣增益低 20 dB，因此指向截获接收机的有效天线增益为：
\[
41.3\,\mathrm{dBi} - 20\,\mathrm{dB} = 21.3\,\mathrm{dBi}
\]

结合 50 dBm 的发射功率，朝向飞机方向的上行链路有效辐射功率（ERP）为：
\[
50\,\mathrm{dBm} + 21.3\,\mathrm{dBi} = 71.3\,\mathrm{dBm}
\]
