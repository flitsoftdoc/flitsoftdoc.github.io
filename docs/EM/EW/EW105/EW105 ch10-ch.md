...
$$
G = -42.2 + 20\log(3) + 20\log(5,000) = -42.2 + 9.5 + 74 = 41.3\,\mathrm{dBi}
$$

其波束宽度为：

$$
\alpha = \operatorname{antilog}\left(\frac{86.8 - 20\log(3) - 20\log(5,000)}{20}\right) = \operatorname{antilog}(3.3/20) = 1.5^{\circ}
$$

若天线指向误差为 $1^{\circ}$，则增益损失为：

$$
\Delta G = 12\left(\frac{1}{1.5}\right)^2 = 5.3\,\mathrm{dB}
$$

则有效天线增益为：

$$
41.3\,\mathrm{dBi} - 5.3\,\mathrm{dB} = 36\,\mathrm{dBi}
$$

故干扰器朝向目标方向的 ERP 为：

$$
50\,\mathrm{dBm} + 36\,\mathrm{dBi} = 86\,\mathrm{dBm}
$$

若直接计算最大 ERP（波束中心）：

$$
ERP = 50\,\mathrm{dBm} + 41.3\,\mathrm{dBi} = 91.3\,\mathrm{dBm}
$$

---

由于目标接收天线为定向天线，干扰与信号功率比（J/S）计算公式为：
...
- $L_J$：干扰链路损耗；
- $L_S$：期望链路损耗；
- $G_{RJ}$：接收天线指向干扰器方向的增益（旁瓣增益）；
- $G_R$：接收天线指向期望信号方向的增益（波束中心增益）。

代入数据：

$$
J/S = 86 - 40 - 158.9 + 132.4 + 11.8 - 31.8 = -0.5\,\mathrm{dB}
$$
...
$$
\begin{aligned}
\alpha &= \operatorname{antilog}\left(\frac{86.8 - 6 - 74}{20}\right) = \operatorname{antilog}\left(\frac{6.8}{20}\right) = 2.2^{\circ}
\end{aligned}
$$
