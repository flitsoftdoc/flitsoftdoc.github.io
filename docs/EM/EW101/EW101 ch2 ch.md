...
因此，对一个球面三角形而言，是由他的弧长与球心角来具体描述的，只是弧的长度是用弧度量来标示。
值得注意的是，球面三角形的三个内角的和总是大于 $180^{\circ}$ ，但在平面上只有 $180^{\circ}$ 。超过 $180^{\circ}$ 的数值称为球面剩余E: $E=\alpha+\beta+\gamma-180^{\circ}$ ，这些结余给出了球面三角形的面积。确定这个值，球面剩余必须以弧度量来测定，表面积 A 依据球面的半径和球面剩余来测量：
