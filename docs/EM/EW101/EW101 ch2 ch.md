...
- 当天线**离地很低**、**频率较低（波长长）**时，第一菲涅尔区的“等效粗壮程度”相对更大，**更容易被地面“擦到”**。此时地面反射与直达波叠加，链路不再像纯自由空间那样按 $d^{-2}$ 衰减，而会出现**双径（two-ray）**主导的干涉/相消趋势，远区渐近到**$d^{-4}$** 衰减。
- 这正是你看到的两段式公式背后的物理图景：**在“菲涅尔区内/前”（尚未进入强地反射干涉的主导区），按自由空间**；**超过某个距离阈值（第一断点/菲涅尔区边界）后**，按**双径远区渐近式**。
///
---
