...
$$
T_{O T}=\frac{K R_{s} \lambda}{2 d_{A} V_{G}|\sin (\alpha)| \cos (\delta)}\tag{SAR.1}
$$
...
| 变量 | AFSIM 来源 | 说明 |
| :--- | :--- | :--- |
| $P_{\text{peak}}$ | 发射机功率 | 发射机的峰值发射功率。 |
| DC | 发射机占空比 | 用户定义的发射机占空比（默认：1.0，如果未定义）。 |
| $\lambda$ |发射机频率或波长 | 辐射信号的波长。 |
| $G_x, G_r$ | 发射机/接收机天线模式 | 发射机和接收机天线的增益。 |
| $L_x, L_r$ | 发射机/接收机内部损耗 | 发射机和接收机的内部损耗。 |
| $A_{xt}, A_{tr}$ | 发射机衰减模型 | 由衰减模型计算得到的单程大气衰减因子（0..1）。 |
| $R_{xt}, R_{tr}$ |计算得出 | 发射机到目标以及目标到接收机的距离。 |
| $\sigma$ |  | 目标的雷达截面积（可以是目标本身或“分辨单元”）。 |
| $F_{40}$ | 发射机传播模型 | 模式传播因子，用于描述直射波与间接反射波之间的相长与相消干涉效应。 |
| $F_{BW}$ |  | 用于描述发射信号带宽与接收机带宽不匹配的因子。这主要用于雷达发射机与被动RF接收机之间，或干扰机与雷达接收机之间的相互作用。它并不用于刻画系统中非理想匹配滤波器的效应。 |
| $F_{\text{pol}}$ | 发射机/接收机极化 | 用于描述发射信号与接收天线之间极化失配的因子。 |
...
还需注意，参考文献1进一步将有效孔径表示为实际孔径面积与"孔径效率"的乘积。在这里我们不会执行这一步，假设任何孔径效率都已在AFSIM的`antenna_pattern`中表示。
...
| $B_N$ | 接收器带宽     | 接收器的带宽。如果带宽没有指定，同时如果指定了发射机脉冲宽度，带宽将计算为 $1/ \mathrm{pulse_width}$（即：将假定为匹配滤波器） |
...
/// note | SAR Dwell Time 
"SAR Dwell Time" 指的是合成孔径雷达（Synthetic Aperture Radar，SAR）在一个特定目标区域或像素上的持续观测时间，也称为驻留时间或凝视时间。在这个时间内，雷达的天线和传感器会持续地收集关于该区域的数据，以便生成更准确和高分辨率的雷达图像。“Dwell Time” 是一个关键参数，因为它影响着雷达图像的质量和分辨率。更长的 Dwell Time 通常会产生更高质量的图像，但这也会增加数据处理的复杂性和时间。
 ///
...
///details | 聚类算法比较
一些 RIPR 智能体会利用 **Cluster Manager** 对威胁或资产的感知对象进行聚类，把较大的目标集合划分为较小的群组来思考与决策。比如，指挥员常把来袭威胁分成两簇，以便让两个中队分别去拦截不同的群组。Cluster Manager 可以基于**相似度阈值**或**期望簇数**进行聚类；相似度度量可选择**地面距离**、**三维距离**，或**三维距离+速度**。它支持三种聚类算法：
- **Hierarchical Tree Max** —— 默认，保证达到最优：任意两个簇内成员之间的“不相似度”不超过阈值（得到更紧致、经典意义上的“团块”式群组）。
- **Hierarchical Tree Min** —— 保证达到最优：每个簇内成员至少与**某一个**成员之间的“不相似度”不超过阈值（允许形成很长、链式的“串联”群组）。
- **K-Means** —— 不保证最优，但最快：簇以 K 个均值点为中心形成。

---

### 三种聚类算法详解

#### 1) Hierarchical Tree Max（层次树 Max，完全链接 / 最大联接）
**核心思想**：以“簇与簇之间的距离 = 两簇中任意两点的最大距离（Max/Complete Link）”为准则，自底向上合并簇。  
**阈值语义**：合并过程在**簇内任意两成员之间的最大不相似度**不超过阈值时才允许；因此最终簇内**所有**点两两都“足够相似”。  
**得到的形态**：紧致、边界清晰、近似“球团/团块”型群组；避免“拉长的链条”。

**典型流程**：
1. 将每个对象初始化为单独一个簇；
2. 计算任意两簇的“最大对内距离”（两簇所有成对点距离的最大值）；
3. 找到距离最小的两簇并尝试合并；若合并后簇内最大成对距离 ≤ 阈值，则合并成立；
4. 重复步骤 2–3，直到所有可合并操作都违反阈值（或直到簇数达到期望的 K）。

**优点**：
- 严格控制簇内“直径”（最大成对距离），簇非常紧致；
- 对“链式效应”鲁棒（不容易被少数近邻点牵出长链）。

**缺点**：
- 计算量大（朴素实现约 $O(n^2)$ 距离与层次合并）；
- 对噪声/离群点敏感（容易提前阻断合并，产生更多小簇）。

**适用建议**：
- 当你需要**保证簇内任意两点都不超过相似度阈值**（例如编队、护航、拦截时要求群内相互支援距离都可控）；
- 当你更关注**紧致、清晰**的群组，而非覆盖更广的链式结构。

---

#### 2) Hierarchical Tree Min（层次树 Min，单链接 / 最小联接）
**核心思想**：以“簇与簇之间的距离 = 两簇中任意两点的最小距离（Min/Single Link）”为准则，自底向上合并簇。  
**阈值语义**：只要簇内每个成员**至少有一个**伙伴在阈值内即可；因此允许“点—点—点”通过近邻关系**串联**起来。  
**得到的形态**：容易形成**细长链条**或“蛇形”群组（chaining effect）。

**典型流程**：
1. 将每个对象初始化为单独一个簇；
2. 计算任意两簇的“最小成对距离”（两簇所有成对点距离的最小值）；
3. 找到距离最小的两簇并尝试合并；若合并后**能保证每个成员至少与某成员在阈值内相似**，则合并成立；
4. 重复步骤 2–3，直到不再满足阈值条件（或簇数达到期望的 K）。

**优点**：
- 能够把稀疏但**局部相邻**的目标沿着“路径”连成一簇，适合识别**队形/编队/车队**等链式结构；
- 对非球形分布友好（可以弯曲、拉长）。

**缺点**：
- 容易被噪声点牵出长链，导致簇“松散”，簇内最远两点可能很远；
- 对“紧致性”需求不友好。

**适用建议**：
- 当你只需要**保证连通**（每个成员至少能与某成员互相支援/通信/感知到），不要求所有成员两两都很近；
- 适合发现**路径状、串联状**目标群。

---

#### 3) K-Means（K 均值）
**核心思想**：给定簇数 K，迭代地把样本分配给最近的“簇中心”，再用簇内样本的均值更新中心，直到收敛。  
**隐含度量**：经典 K-Means 等价于**欧氏距离**最小化。若使用“3D 距离 + 速度”，常见做法是将位置与速度拼为向量，并对不同量纲做**归一化/加权**。

**典型流程**：
1. 设定簇数 K（来自任务需要，如“把威胁分成两群给两个中队”）；
2. 初始化 K 个中心（随机或 K-Means++）；
3. **分配**：按距离最近把样本分到各中心；
4. **更新**：用各簇样本的均值更新中心；
5. 重复 3–4，直到中心变化很小或达到迭代上限。

**优点**：
- **速度快、实现简单**，适合大样本在线更新；
- 直接满足“需要 K 个群”的编组需求（如战术上明确要 2 群或 3 群）。

**缺点**：
- 只保证**局部最优**，结果受初始化影响；
- 偏好**类球形**簇，对链式或非凸簇表现差；
- **不直接支持阈值约束**（可在后处理检查类内直径，超阈值则调整 K 或改用层次法）。

**适用建议**：
- 当任务明确**指定簇数 K** 且需要**快速**划分时（例如来袭威胁快速分群分配拦截资源）；
- 如果对**严格阈值**或非凸形簇有要求，优先考虑层次法或混合策略。
...
- **相似度/距离**：可选择  
  - 地面距离（投影到地面的 2D 距离）；  
  - 三维距离（考虑高度差）；  
  - 三维距离 + 速度（位置速度联合特征，需归一化/加权）。  
- **按阈值聚类**：  
  - 需要“簇内任意两点都不超阈值” ⇒ **Hierarchical Tree Max**；  
  - 需要“每点至少与一近邻不超阈值” ⇒ **Hierarchical Tree Min**。  
- **按簇数聚类**：  
  - 明确指定 K（如两群） ⇒ **K-Means**，必要时再做阈值合规检查。

---

### 任务场景选择建议

- **两群快速编组拦截**：K-Means (K=2) 速度快，便于实时指挥；若簇内直径超限，可微调 K 或切换层次法。  
- **需要簇内“全体互相支援距离”受控**：Hierarchical Tree Max（保证任意两点不超阈值，簇更紧致）。  
- **识别长队形/串联目标**：Hierarchical Tree Min（允许“链式”连接，保持连通）。
