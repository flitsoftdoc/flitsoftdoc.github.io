...
这篇综合性文章**探讨了大型语言模型（LLM）中“思考时间”或“测试时间计算”的重要性**，旨在通过各种技术增强模型性能。文章首先将模型推理比作人类的快思慢想，**强调了通过链式思考（CoT）等方法延长处理时间的益处**。它详细介绍了**两种主要的解码策略：并行采样和顺序修订**，以优化模型输出。此外，文章**深入分析了强化学习（RL）在提升推理能力方面的应用**，包括使用外部工具和评估模型“思考”的忠实性。最后，它**讨论了在连续空间中进行思考的架构创新**以及**思维时间和计算扩展的潜在扩展定律**，展望了未来AI系统可能模仿人类思考模式的发展方向。

**音频概览：**

<audio controls>
  <source src="/assets/audio/why we think.m4a" type="audio/mpeg" />
  您的浏览器不支持音频播放。
</audio>
